...
# HOST = '172.20.123.64'  # Standard loopback interface address (localhost)
PORT = 1397  # Port to listen on (non-privileged ports are > 1023)
					...
					while True:
						for event in pygame.event.get():
							if event.type == QUIT:
								pygame.quit()
								exit()
				conn.send(bytes(json.dumps(1), 'utf-8'))


# @atexit.register
# def f():
# 	s.close()
# 	print(f'Connection {addr} was closed...')
